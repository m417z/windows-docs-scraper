...
- **FileObject** [in]: [**FltFsControlFile**](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nf-fltkernel-fltfscontrolfile) only. The file pointer object specifying the file to write to. This parameter is required and cannot be NULL.

- **FileHandle** [in]: [**ZwFsControlFile**](https://learn.microsoft.com/previous-versions/ff566462(v=vs.85)) only. The file handle of the file to write to. This parameter is required and cannot be NULL.

- **FsControlCode** [in]: The control code for the operation. Use **FSCTL_OFFLOAD_WRITE** for this operation.

- **InputBuffer** [in]: A pointer to a [**FSCTL_OFFLOAD_WRITE_INPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_input) structure, which contains the size and offset of a data block to read.
...
- **OutputBuffer** [out]: A pointer to a [**FSCTL_OFFLOAD_WRITE_INPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_input) structure, which contains the size and offset of a data block to read.
...
| STATUS_END_OF_FILE | The **FileOffset** member of[**FSCTL_OFFLOAD_WRITE_INPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_input) begins after end-of-file (EOF). |
...
| STATUS_BUFFER_TOO_SMALL | **InputBufferLength** is too small for **InputBuffer** to contain an[**FSCTL_OFFLOAD_WRITE_INPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_input) structure, or **OutputBufferLength** is too small for **OutputBuffer** to receive an [**FSCTL_OFFLOAD_WRITE_OUTPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_output) structure. |
...
- One or more of these members of[**FSCTL_OFFLOAD_WRITE_INPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_input) are incorrect:
...
**Size** is not the size of the[**FSCTL_OFFLOAD_WRITE_INPUT**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/ns-ntifs-_fsctl_offload_write_input) structure.
