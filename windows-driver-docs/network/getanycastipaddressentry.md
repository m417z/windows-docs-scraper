...
- *Row* \[in, out\]
A pointer to a [**MIB\_ANYCASTIPADDRESS\_ROW**](https://learn.microsoft.com/windows-hardware/drivers/network/mib-anycastipaddress-row) structure entry for an anycast IP address entry. On successful return, this structure is updated with the properties for an existing anycast IP address.
...
The **GetAnycastIpAddressEntry** function is used to retrieve an existing [**MIB\_ANYCASTIPADDRESS\_ROW**](https://learn.microsoft.com/windows-hardware/drivers/network/mib-anycastipaddress-row) structure entry.
...
Your driver can call the [**GetAnycastIpAddressTable**](https://learn.microsoft.com/windows-hardware/drivers/network/getanycastipaddresstable) function to enumerate the anycast IP address entries on a local computer.
