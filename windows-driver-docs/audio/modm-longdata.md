...
WINMM sends the `MODM_LONGDATA` message to the [**modMessage**](https://learn.microsoft.com/windows-hardware/drivers/audio/mod-message) function of a MIDI output driver when a client application wants the driver to make a data block available as output. This data block typically contains one or more MIDI events, including system-exclusive events. If the data block contains more than one MIDI event, the events are packed into the data block with no padding.

If a client of the MIDI output driver wants to send a single MIDI event, it is more efficient to use the [**MODM\_DATA**](https://learn.microsoft.com/windows-hardware/drivers/audio/modm-data) message.
...
- *dwParam1*
This parameter specifies a far pointer to [MIDIHDR](https://learn.microsoft.com/windows/win32/api/mmeapi/ns-mmeapi-midihdr) data structure that identifies the data block.
