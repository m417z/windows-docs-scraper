...
> [!Note]
> This function is used only by applications that run on pre-Windows Vista operating systems. Its use requires the download package. Applications that only run on Windows Vista and later should call [**GetStringScripts**](https://learn.microsoft.com/windows/desktop/api/Winnls/nf-winnls-getstringscripts).
