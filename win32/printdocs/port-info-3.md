...
When you set a printer port status value with the severity value PORT\_STATUS\_TYPE\_ERROR, the print spooler stops sending jobs to the port. The print spooler does not resume sending jobs to the port until another [**SetPort**](https://learn.microsoft.com/windows/win32/printdocs/setport) call is made to clear the status.
