...
In this short example note that no function parameters are declared, that the number of inputs and type of each input is declared before the entry function, the input is retrieved by calling [D2DGetInput](https://learn.microsoft.com/windows/win32/direct2d/d2dgetinput), and that preprocessor directives must be defined before the helper file is included.
