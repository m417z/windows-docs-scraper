...
Touch and pen input use different window messages than mouse input. See [Pointer Input](https://learn.microsoft.com/windows/win32/api/_inputmsg/) for more info. By default, apps that do not handle pointer messages have the primary pointer contact automatically promoted to mouse input. Apps that do handle pointer messages do not receive mouse messages, which can cause problems when calling APIs that expect mouse input, such as [**DoDragDrop**](https://learn.microsoft.com/windows/win32/api/ole2/nf-ole2-dodragdrop).
...
The calling thread must have exactly one pointer contact, and it must be primary. See [**IS_POINTER_PRIMARY_WPARAM**](https://learn.microsoft.com/windows/win32/api/winuser/nf-winuser-is_pointer_primary_wparam).
