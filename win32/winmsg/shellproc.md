...
An application-defined or library-defined callback function used with the [**SetWindowsHookExA**](https://learn.microsoft.com/windows/win32/api/winuser/nf-winuser-setwindowshookexa)/[**SetWindowsHookExW**](https://learn.microsoft.com/windows/win32/api/winuser/nf-winuser-setwindowshookexw) function. The function receives notifications of Shell events from the system.
...
| **HSHELL_APPCOMMAND** 12 | The user completed an input event (for example, pressed an application command button on the mouse or an application command key on the keyboard), and the application did not handle the [WM_APPCOMMAND](https://learn.microsoft.com/windows/desktop/inputdev/wm-appcommand) message generated by that input. If the Shell procedure handles the [WM_COMMAND](https://learn.microsoft.com/windows/desktop/menurc/wm-command) message, it should not call **CallNextHookEx**. See the Return Value section for more information. |
...
| **HSHELL_GETMINRECT** | A pointer to a [RECT](https://learn.microsoft.com/windows/win32/api/windef/ns-windef-rect) structure. |
...
The return value should be zero unless the value of *nCode* is **HSHELL_APPCOMMAND** and the shell procedure handles the [**WM_COMMAND**](https://learn.microsoft.com/windows/win32/menurc/wm-command) message. In this case, the return should be nonzero.

## Remarks

Install this hook procedure by specifying the [WH_SHELL](https://learn.microsoft.com/windows/win32/winmsg/about-hooks) hook type and a pointer to the hook procedure in a call to the **SetWindowsHookEx** function.
...
[WM_APPCOMMAND](https://learn.microsoft.com/windows/desktop/inputdev/wm-appcommand)
