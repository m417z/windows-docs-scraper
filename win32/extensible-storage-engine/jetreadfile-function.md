...
The **JetReadFile** function retrieves the contents of a file opened with [JetOpenFile](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetopenfile-function).
...
The operation failed because the current external backup has been aborted by a call to [JetStopService](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg269240(v=exchg.10).md). This error will only be returned by Windows XP and later releases.
...
* The output buffer size is not a multiple of the database page size ([JET_paramDatabasePageSize](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg269337(v=exchg.10).md)). Windows XP and later releases.
* The output buffer size is smaller than three database pages ([JET_paramDatabasePageSize](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg269337(v=exchg.10).md)) and this is the first call to **JetReadFile** for the specified handle. Windows XP and later releases.
