...
One of the parameters provided contained an unexpected value or contained a value that did not make sense when combined with the value of another parameter. This error will be returned by [JetRestoreInstance](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg269306(v=exchg.10).md) when the engine is in multi-instance mode and pinstance refers to an invalid instance Windows XP and later releases.
...
[JetRestore](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetrestore-function) functions must be called on an instance before [JetInit](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetinit-function) is called for that instance.
...
Implemented as **JetRestore2W** (Unicode) and **JetRestore2A** (ANSI).
