...
**Windows 2000:** In Windows 2000, any database attached to the instance during a previous incarnation of that instance remains attached to the instance after **JetInit** completes successfully. It is not necessary to call [JetAttachDatabase](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetattachdatabase-function) after **JetInit** to insure later database access. If the [JetAttachDatabase](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetattachdatabase-function) function is called after the **JetInit** function, the JET_wrnDatabaseAttached warning will be returned. This warning indicates that the database attachment was preserved and can be ignored.

**Windows XP:** In Windows XP and later releases, all databases are automatically detached from the instance by **JetInit**. This means that [JetAttachDatabase](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetattachdatabase-function) must always be called after **JetInit** in this case.

Any application written to run on Windows 2000 and on later releases must always call [JetAttachDatabase](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetattachdatabase-function) after **JetInit**. If the application runs on Windows 2000 then it must expect to see JET_wrnDatabaseAttached in some cases. See [JetAttachDatabase](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetattachdatabase-function) for more information.
...
[JET_paramMaxTemporaryTables](https://learn.microsoft.com/windows/win32/extensible-storage-engine/temporary-database-parameters)
[JET_paramRecovery](https://learn.microsoft.com/windows/win32/extensible-storage-engine/transaction-log-parameters)
