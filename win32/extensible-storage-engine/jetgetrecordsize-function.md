...
A pointer to an output buffer for the [JET_RECSIZE](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jet-recsize-structure) structure.
...
When this bit is specified, the [JET_RECSIZE](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg294072(v=exchg.10).md) is not zeroed before filling the contents, effectively acting as an accumulation of the statistics for multiple records visited or updated.
...
**Windows XP:** JET_errInstanceUnavailable will only be returned by Windows XP and later releases.
...
The size of the key accumulated in the **cbOverhead** field of [JET_RECSIZE](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jet-recsize-structure), is affected by JET_bitRecordSizeInCopyBuffer. If this bit is specified, the key size accumulated in the **cbOverhead** field is the full key size. If this bit is not used, then the key size accumulated will not include any size saved due to key prefix compression.
