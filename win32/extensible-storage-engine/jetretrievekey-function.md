...
The **JetRetrieveKey** function retrieves the key for the index entry at the current position of a cursor. Such keys are constructed by calls to [JetMakeKey](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetmakekey-function). The retrieved key can then be used to efficiently return that cursor to the same index entry by a call to [JetSeek](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetseek-function).
...
When specified, the engine will return the search key for the cursor. The search key is built up using one or more prior calls to [JetMakeKey](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg269329(v=exchg.10).md) for the purposes of seeking to that key using [JetSeek](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg294103(v=exchg.10).md) or setting an index range using [JetSetIndexRange](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg294112(v=exchg.10).md).
...
There is no current search key for the cursor. This will happen for **JetRetrieveKey** if JET_bitRetrieveCopy is specified and a search key has not been constructed for this cursor using a prior call to [JetMakeKey](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg269329(v=exchg.10).md). The search key will be deleted by a prior call to any navigational API on the cursor other than [JetMove](https://learn.microsoft.com/windows/win32/extensible-storage-engine/gg294117(v=exchg.10).md).
...
The operation completed successfully, but the output buffer was too small to receive the entire key. The output buffer has been filled with as much of the key as would fit. The actual size of the key has also been returned, if requested.

**Note** This error will not be returned if JET_bitRetrieveCopy is specified. Please see the Remarks section for more information.
...
In addition to the above properties of ESENT keys in general, it is important to note that a search key is different from the key for an index entry. Specifically, a search key may be longer than an ordinary key. This extra length occurs when a wildcard option is used while constructing the search key. See [JetMakeKey](https://learn.microsoft.com/windows/win32/extensible-storage-engine/jetmakekey-function) for more information.
