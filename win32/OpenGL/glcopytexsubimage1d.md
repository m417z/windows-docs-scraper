...
| **GL\_INVALID\_VALUE** | *level* was less than zero or *level* is greater than *log*2(*max*), where *max* is the returned value of GL\_MAX\_TEXTURE\_SIZE.<br> |
| **GL\_INVALID\_VALUE** | *xoffset* was less than *border* or (*xoffset* + *width*)was greater than (*w* + *border*), where *w* is GL\_TEXTURE\_WIDTH and *border* is GL\_TEXTURE\_BORDER. Note that *w* includes twice the *border* width.<br> |
...
The **glCopyTexSubImage1D** function replaces a portion of a one-dimensional texture image using pixels from the current framebuffer, rather than from main memory as is the case for [**glTexSubImage1D**](https://learn.microsoft.com/windows/win32/opengl/gltexsubimage1d).
