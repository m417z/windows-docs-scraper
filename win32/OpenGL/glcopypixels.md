...
| Value | Meaning |
|-------|---------|
| **GL_COLOR** | The **glCopyPixels** function reads indexes or RGBA colors from the buffer currently specified as the read source buffer (see [**glReadBuffer**](https://learn.microsoft.com/windows/win32/opengl/glreadbuffer)). <br> If OpenGL is in color-index mode:<br>
