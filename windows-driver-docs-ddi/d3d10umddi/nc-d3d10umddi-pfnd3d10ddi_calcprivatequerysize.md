...
A pointer to a [D3D10DDIARG_CREATEQUERY](https://learn.microsoft.com/windows-hardware/drivers/ddi/d3d10umddi/ns-d3d10umddi-d3d10ddiarg_createquery) structure that describes the parameters that the user-mode display driver uses to calculate the size of the memory region.

## Return value

**CalcPrivateQuerySize** returns the size of the memory region that the driver requires for creating a query.
