...
The total number of constant buffers. The number can be -1, which specifies that the Direct3D runtime uses its high watermarks to substitute an optimal value (which is typically less than the maximum valid value for *Count*). However, no non-NULL binding exists in a slot larger than the optimal *Count* value.
