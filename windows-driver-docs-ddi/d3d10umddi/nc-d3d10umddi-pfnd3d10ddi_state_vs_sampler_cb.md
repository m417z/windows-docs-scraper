...
The total number of DDI handles to samplers. The number can be -1, which specifies that **pfnStateVsSamplerCb** will use its high watermarks to substitute an optimal value (which is typically less than the maximum valid value for *Count*). However, no non-NULL binding exists in a slot larger than the optimal *Count* value.
