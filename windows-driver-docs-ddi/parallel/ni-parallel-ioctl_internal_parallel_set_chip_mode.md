...
The **AssociatedIrp.SystemBuffer** member points to a [PARALLEL_CHIP_MODE](https://learn.microsoft.com/windows-hardware/drivers/ddi/parallel/ns-parallel-_parallel_chip_mode) structure that the client allocates to input chip mode information. The client sets the **ChipMode** member to the requested operating mode.
