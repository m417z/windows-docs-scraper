...
**FltGetBottomInstance** adds a rundown reference to the opaque instance pointer returned in **Instance*. When this pointer is no longer needed, the caller must release it by calling [FltObjectDereference](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nf-fltkernel-fltobjectdereference). Thus every successful call to **FltGetBottomInstance** must be matched by a subsequent call to **FltObjectDereference**.
