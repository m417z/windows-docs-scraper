...
A bitmask for the associated file I/O operation. A minifilter driver sets bits to specify the behavior of **FltOplockBreakToNoneEx**. The *Flags* parameter has the following options:

#### OPLOCK_FLAG_COMPLETE_IF_OPLOCKED (0x00000001)

Allows an oplock break to proceed without blocking or pending the operation that caused the oplock break. Typically, this flag is only used if the I/O operation that is represented by the callback data that the *CallbackData* parameter points to is an IRP_MJ_CREATE operation.
...
A context information pointer that was passed in the *Context* parameter to **FltOplockBreakToNoneEx**.
...
**FltOplockBreakToNoneEx** returns one of the following FLT_PREOP_CALLBACK_STATUS codes:

| Return code | Description |
| --- | --- |
| **FLT_PREOP_COMPLETE** | **FltOplockBreakToNoneEx** encountered a pool allocation failure, or a call to the [FsRtlOplockBreakToNoneEx](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-_fsrtl_advanced_fcb_header-fsrtloplockbreaktononeex) function returned an error. **FltOplockBreakToNoneEx** will set the error code in the **Status** member of the [IO_STATUS_BLOCK](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/ns-wdm-_io_status_block) structure. The IO_STATUS_BLOCK structure is specified in the **IoStatus** member of the [FLT_CALLBACK_DATA](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/ns-fltkernel-_flt_callback_data) callback data structure. The *CallbackData* parameter points to this FLT_CALLBACK_DATA. |
...
[FsRtlOplockBreakToNoneEx](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-_fsrtl_advanced_fcb_header-fsrtloplockbreaktononeex)
