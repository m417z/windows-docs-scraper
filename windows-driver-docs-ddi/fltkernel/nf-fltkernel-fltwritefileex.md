...
**FltWriteFileEx** is used to write data to an open file, stream, or device. This function extends [**FltWriteFile**](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nf-fltkernel-fltwritefile) to allow the optional use of an MDL for write data instead of a mapped buffer address.
...
A pointer to a [**PFLT_COMPLETED_ASYNC_IO_CALLBACK**](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nc-fltkernel-pflt_completed_async_io_callback)-typed callback routine to call when the write operation is complete. This parameter is optional and can be NULL.
...
[**FltReadFileEx**](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nf-fltkernel-fltreadfileex)
