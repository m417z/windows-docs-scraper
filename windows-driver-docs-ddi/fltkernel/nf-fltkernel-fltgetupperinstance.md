...
| --- | --- |
| **STATUS_NO_MORE_ENTRIES** | No higher instance was found. This is a warning code. |
...
**FltGetUpperInstance** adds a rundown reference to the opaque instance pointer returned in the *UpperInstance* parameter. When this pointer is no longer needed, the caller must release it by calling [FltObjectDereference](https://learn.microsoft.com/windows-hardware/drivers/ddi/fltkernel/nf-fltkernel-fltobjectdereference). Thus every successful call to **FltGetUpperInstance** must be matched by a subsequent call to **FltObjectDereference**.
