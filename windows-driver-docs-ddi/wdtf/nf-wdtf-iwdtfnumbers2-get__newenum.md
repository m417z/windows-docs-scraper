# IWDTFNumbers2::get__NewEnum

## Description

Gets a new iteration variable that the **For Each**
loop structure implicitly uses.

This property is read-only.

## Parameters

### `ppNewEnum`

## Return value

## Remarks

You cannot access the **_NewEnum** property from the JScript programming language.

You must use the Active Template Library (ATL) to implement this functionality.
For more information about how ATL implements this functionality, see the
**ICollectionOnSTLImpl** interface in the MSDN Library.

## See also

[IWDTFNumbers2](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdtf/nn-wdtf-iwdtfnumbers2)