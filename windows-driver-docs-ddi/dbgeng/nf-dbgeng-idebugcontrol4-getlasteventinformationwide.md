...
Receives extra information about the event. The contents of this extra information depends on the type of the event as indicated by the returned *Type* parameter. For example, if *Type* is breakpoint, *ExtraInformation* contains a DEBUG_LAST_EVENT_INFO_BREAKPOINT; if Type is Exception, ExtraInformation contains a DEBUG_LAST_EVENT_INFO_EXCEPTION. Refer to [DEBUG_EVENT_XXX](https://learn.microsoft.com/previous-versions/ff541478(v=vs.85)) for the complete list of event types and the dbgeng.h header file for the structure definitions for each event type.
