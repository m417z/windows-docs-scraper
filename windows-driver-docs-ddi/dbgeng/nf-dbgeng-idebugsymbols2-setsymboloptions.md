...
Specifies the new symbol options. *Options* is a bit-set; it will replace the existing symbol options. For a description of the bit flags, see [Setting Symbol Options](https://learn.microsoft.com/windows-hardware/drivers/debugger/symbol-options).
...
This method will set the engine's global symbol options to those specified in *Options*. Unlike [AddSymbolOptions](https://learn.microsoft.com/windows-hardware/drivers/ddi/dbgeng/nf-dbgeng-idebugsymbols3-addsymboloptions), any symbol options not in the bit-set *Options* will be removed.
