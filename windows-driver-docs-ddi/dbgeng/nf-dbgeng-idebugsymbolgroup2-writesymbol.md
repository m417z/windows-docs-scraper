...
The **WriteSymbol** method can change symbols only if the symbols are stored in a register or memory location that the [debugger engine](https://learn.microsoft.com/windows-hardware/drivers/debugger/introduction) knowns and if they have not had their type changed to an extension by using the [OutputAsType](https://learn.microsoft.com/windows-hardware/drivers/ddi/dbgeng/nf-dbgeng-idebugsymbolgroup2-outputastype) method.
