...
**NtCreateSectionEx** creates a [section object**](https://learn.microsoft.com/windows-hardware/drivers/kernel/section-objects-and-views) and opens a handle to the object with the specified desired access. It is an extended version of [**NtCreateSection**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-ntcreatesection) that creates a section object by specifying to indicate that this is actually an Address Windowing Extensions (AWE) section.
...
A pointer to an array of [**MEM_EXTENDED_PARAMETER**](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/ns-wdm-mem_extended_parameter) structures the contains the extended parameters to create the section.
...
Once the handle pointed to by **SectionHandle** is no longer in use, the driver must call [**NtClose**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-ntclose) to close it.
...
For more information about setting up mapped sections and views of memory, see [Sections and Views](https://learn.microsoft.com/windows-hardware/drivers/ddi/_kernel/#sections-and-views).
