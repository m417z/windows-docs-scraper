...
The **ZwDuplicateToken** function creates a handle to a new [access token](https://learn.microsoft.com/windows/win32/secgloss/a-gly) that duplicates an existing token. This function can create either a primary token or an impersonation token.
...
For additional information, see [Access Rights for Access-Token Objects](https://learn.microsoft.com/windows/win32/secauthz/access-rights-for-access-token-objects) in the Windows SDK. Note that access tokens do not support the SYNCHRONIZE right.
...
For information on the user-mode analog of **ZwDuplicateToken**, see [**DuplicateTokenEx**](https://learn.microsoft.com/windows/win32/api/securitybaseapi/nf-securitybaseapi-duplicatetokenex).

When you have finished using the new token, call the [**ZwClose**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntifs/nf-ntifs-ntclose) function to close the token handle.
