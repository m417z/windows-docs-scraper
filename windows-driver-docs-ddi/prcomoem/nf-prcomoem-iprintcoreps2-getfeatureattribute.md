...
Pointer to a caller-supplied buffer containing an ASCII string specifying the single feature keyword to query for. This value can be obtained from a prior call to [IPrintCorePS2::EnumFeatures](https://learn.microsoft.com/windows-hardware/drivers/ddi/prcomoem/nf-prcomoem-iprintcoreps2-enumfeatures).
...
| **E_INVALIDARG** | The method attempted to query for a nonexistent attribute.<br><br>The feature keyword was not recognized.<br><br>The *pdevobj* parameter pointed to an invalid driver context object. |
