...
A [**DXGK_DSI_TRANSMISSION**](https://learn.microsoft.com/windows-hardware/drivers/ddi/dispmprt/ns-dispmprt-dxgk_dsi_transmission) structure followed by [**DXGK_DSI_PACKET**](https://learn.microsoft.com/windows-hardware/drivers/ddi/dispmprt/ns-dispmprt-dxgk_dsi_packet) structures containing the packets.
...
The OS will ensure that the sequence of DSI packets is well-formed, with valid info in all defined fields and correct buffer sizes. The OS is responsible for initializing the **FailedPacket** field to DXGK_DSI_INVALID_PACKET_INDEX so that further validation in the OS or driver only needs to set the field if a problem is found with a particular packet. If the [**DXGK_DSI_TRANSMISSION**](https://learn.microsoft.com/windows-hardware/drivers/ddi/dispmprt/ns-dispmprt-dxgk_dsi_transmission) structure is not well-formed, the OS will set the DXGK_HOST_DSI_INVALID_TRANSMISSION flag in the **HostErrors** field to distinguish this from other invalid params errors.
...
Hex | Command
--- | -------
01h | soft_reset - note, this must be sent via an [IOCTL_MIPI_DSI_RESET](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddvdeo/ni-ntddvdeo-ioctl_mipi_dsi_reset)
...
If the transmission passes OS validation, the OS passes the buffer to the graphics driver using [**DsiTransmission**](https://learn.microsoft.com/windows-hardware/drivers/ddi/dispmprt/nc-dispmprt-dxgkddi_dsitransmission).
...
[**DXGK_DSI_TRANSMISSION**](https://learn.microsoft.com/windows-hardware/drivers/ddi/dispmprt/ns-dispmprt-dxgk_dsi_transmission)
