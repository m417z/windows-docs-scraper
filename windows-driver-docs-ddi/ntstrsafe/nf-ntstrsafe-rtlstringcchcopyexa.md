...
**RtlStringCchCopyExW** and **RtlStringCchCopyExA** add to the functionality of [RtlStringCchCopy](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntstrsafe/nf-ntstrsafe-rtlstringcchcopya) by returning a pointer to the end of the destination string, as well as the number of characters left unused in that string. Flags can be passed to the function for additional control. Use **RtlStringCchCopyExW** to handle Unicode strings and **RtlStringCchCopyExA** to handle ANSI strings. The form you use depends on your data, as shown in the following table.
