...
The first element in an array of [PO_FX_PERF_STATE_CHANGE](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/ns-wdm-_po_fx_perf_state_change) structures that represent the performance states the driver intends to transition to. Each array element represents a single performance state change request.
...
A driver calls **PoFxIssueComponentPerfStateChangeMultiple**, the power management framework (PoFx) will request the platform extension plug-in (PEP) to place
