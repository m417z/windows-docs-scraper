...
The byte alignment of the data entries pointed to by *rgWriteEntries* as they are marshaled into a single record. A value of one specifies simple concatenation (see [ClfsReserveAndAppendLog](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-clfsreserveandappendlog)). A value larger than one can result in zeros being placed between entries in the record. The value of this parameter must be greater than zero.
...
| CLFS_FLAG_USE_RESERVATION | The current record is placed in reserved space in an I/O block. The number of reserved records in the marshalling area is reduced by one. If this flag is set, *cReserveRecords* must be zero and *rgcbReservation* must be **NULL**. |
...
**ClfsReserveAndAppendLogAligned** returns STATUS_SUCCESS if it succeeds; otherwise, it returns one of the error codes defined in Ntstatus.h.

## Remarks

The **ClfsReserveAndAppendLogAligned** routine changes its fundamental behavior based on the presence of optional parameters and the state of the CLFS_USE_RESERVATION flag. The following table summarizes common scenarios.
