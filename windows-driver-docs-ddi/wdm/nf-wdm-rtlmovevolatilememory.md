...
The **RtlMoveVolatileMemory** function exists to provide [**RtlMoveMemory**](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-rtlmovememory) behavior (for example, copying memory from one location to another) in situations where the developer needs to be sure that the copy operation occurs (for example, isn't subject to compiler optimizations). Unlike [**RtlCopyVolatileMemory**](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-rtlcopyvolatilememory), this function handles cases where the *Source* and *Destination* buffer overlap.
...
[**RtlCopyVolatileMemory**](https://learn.microsoft.com/windows-hardware/drivers/ddi/wdm/nf-wdm-rtlcopyvolatilememory)
