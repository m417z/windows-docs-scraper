...
| **CM_RESOURCE_INTERRUPT_SECONDARY_INTERRUPT** | The interrupt is capable of waking the operating system from a low-power idle state or a system sleep state. This flag can be set starting with Windows 8. For more information about wake capabilities, see [Enabling Device Wake-Up](https://learn.microsoft.com/windows-hardware/drivers/kernel/enabling-device-wake-up). |
