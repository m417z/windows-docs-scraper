...
The ATA_PASS_THROUGH_EX structure is used in conjunction with an [IOCTL_ATA_PASS_THROUGH](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_ata_pass_through) request to instruct the port driver to send an embedded ATA command to the target device.
...
When [IOCTL_ATA_PASS_THROUGH](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_ata_pass_through) completes, the port driver updates **CurrentTaskFile** with the values that are present in the device's output registers at the completion of the embedded command. The array values in **CurrentTaskFile** correspond to the following task file output registers.
...
[IOCTL_ATA_PASS_THROUGH](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_ata_pass_through) is a buffered device control request. To bypass buffering in system memory, callers should use [IOCTL_ATA_PASS_THROUGH_DIRECT](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_ata_pass_through_direct) and [ATA_PASS_THROUGH_DIRECT](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ns-ntddscsi-_ata_pass_through_direct). When handling an IOCTL_ATA_PASS_THROUGH_DIRECT request, the system locks down the buffer in user memory and the device accesses this memory directly.
