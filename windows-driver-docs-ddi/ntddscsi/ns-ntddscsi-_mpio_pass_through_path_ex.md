...
The **MPIO_PASS_THROUGH_PATH_EX** structure is used together with an [**IOCTL_MPIO_PASS_THROUGH_PATH_EX**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_mpio_pass_through_path_ex) request to instruct the port driver to send an embedded SCSI command to the target device.

## Members

### `PassThroughOffset`

The offset from the beginning of this structure to a [**SCSI_PASS_THROUGH_EX**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ns-ntddscsi-_scsi_pass_through_ex) structure that is configured in the same manner as it is for an [**IOCTL_SCSI_PASS_THROUGH_EX**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_scsi_pass_through_ex) request.
...
The port number if MPIO_IOCTL_FLAG_USE_SCSIADDRESS is set. Otherwise, this member is zero. If MPIO_IOCTL_FLAG_USE_SCSIADDRESS is set, the **PathID** and **TargetId** values are taken from address structure associated with the embedded [**SCSI_PASS_THROUGH_EX**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ns-ntddscsi-_scsi_pass_through_ex) structure.
...
> [!NOTE]
> All 32-bit processes running on a 64-bit version of Windows must use the **MPIO_PASS_THROUGH_PATH32_EX** structure when issuing an [**IOCTL_MPIO_PASS_THROUGH_PATH_EX**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_mpio_pass_through_path_ex) request.
...
[**IOCTL_MPIO_PASS_THROUGH_PATH_EX**](https://learn.microsoft.com/windows-hardware/drivers/ddi/ntddscsi/ni-ntddscsi-ioctl_mpio_pass_through_path_ex)
