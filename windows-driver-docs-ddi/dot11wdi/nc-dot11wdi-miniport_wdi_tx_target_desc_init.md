...
The
*MINIPORT_WDI_TX_TARGET_DESC_INIT* callback function associates an opaque target TX descriptor with the [NET_BUFFER_LIST](https://learn.microsoft.com/windows-hardware/drivers/ddi/nbl/ns-nbl-net_buffer_list) (**MiniportReserved[1]** field) and (if applicable) populates the TX cost field (in credit units) in the [WDI_FRAME_METADATA](https://learn.microsoft.com/windows-hardware/drivers/ddi/dot11wdi/ns-dot11wdi-_wdi_frame_metadata) buffer of the **NET_BUFFER_LIST** (**MiniportReserved[0]**).
...
**Note** You must declare the function by using the **MINIPORT_WDI_TX_TARGET_DESC_INIT** type. For more
...
Pointer to a single [NET_BUFFER_LIST](https://learn.microsoft.com/windows-hardware/drivers/ddi/nbl/ns-nbl-net_buffer_list).
...
A status of **NDIS_STATUS_RESOURCES** must only be used when Tx descriptors are exhausted. The best behavior occurs if the IHV miniport does not use **NDIS_STATUS_RESOURCES**. Any status other than **NDIS_STATUS_SUCCESS** and **NDIS_STATUS_RESOURCES** must be used for any other error that indicates the IHV miniport or firmware is unable to process the [NET_BUFFER_LIST](https://learn.microsoft.com/windows-hardware/drivers/ddi/nbl/ns-nbl-net_buffer_list).

The IHV miniport may utilize the backfill in the [NET_BUFFER](https://learn.microsoft.com/windows-hardware/drivers/ddi/nbl/ns-nbl-net_buffer) by using the appropriate NDIS retreat and advance functions. The start of the packet offset must be restored before or in the [MINIPORT_WDI_TX_TARGET_DESC_DEINIT](https://learn.microsoft.com/windows-hardware/drivers/ddi/dot11wdi/nc-dot11wdi-miniport_wdi_tx_target_desc_deinit) callback function. The TxMgr determines the frame length during dequeue time so any change to the frame length in the *MINIPORT_WDI_TX_TARGET_DESC_INIT* callback function is included in limiting the dequeue size to the quantum. In this case, the IHV miniport should appropriately adjust the quantum to account for the delta.

**Important** The IHV miniport is not permitted to make any indication in the context of this call.
The TxMgr serializes calls to [TxTargetDescInitHandler](https://learn.microsoft.com/windows-hardware/drivers/ddi/dot11wdi/ns-dot11wdi-_ndis_miniport_wdi_data_handlers).
...
[MINIPORT_WDI_TX_TARGET_DESC_DEINIT](https://learn.microsoft.com/windows-hardware/drivers/ddi/dot11wdi/nc-dot11wdi-miniport_wdi_tx_target_desc_deinit)
