...
The parameters for a provision discovery request send complete are specified in a **DOT11_PROVISION_DISCOVERY_REQUEST_SEND_COMPLETE_PARAMETERS** structure. This structure is sent with an [NDIS_STATUS_DOT11_WFD_PROVISION_DISCOVERY_REQUEST_SEND_COMPLETE](https://learn.microsoft.com/windows-hardware/drivers/network/ndis-status-dot11-wfd-provision-discovery-request-send-complete) indication.

## Members

### `Header`

The type, revision, and size of the **DOT11_PROVISION_DISCOVERY_REQUEST_SEND_COMPLETE_PARAMETERS** structure. The required settings for the members of **Header** are the following.
...
The dialog token used for the provision discovery request packet. This token must match the token sent with the [OID_DOT11_WFD_SEND_PROVISION_DISCOVERY_REQUEST](https://learn.microsoft.com/windows-hardware/drivers/network/-oid-dot11-wfd-send-provision-discovery-request) request.
