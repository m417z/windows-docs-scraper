...
The returned *securityDescriptor* parameter must be freed through a call to **FwpmFreeMemory0[FwpmFreeMemory0](https://learn.microsoft.com/windows-hardware/drivers/ddi/fwpmk/nf-fwpmk-fwpmfreememory0)**. The other four returned parameters must not be freed, as they point to addresses within the *securityDescriptor* parameter.
