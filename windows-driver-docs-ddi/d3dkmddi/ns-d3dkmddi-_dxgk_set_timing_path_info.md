...
Structure to hold information to modify [**SetTiming**](https://learn.microsoft.com/windows-hardware/drivers/ddi/d3dkmddi/ns-d3dkmddi-_dxgkarg_settimingsfromvidpn) path.
...
A [**D3DKMDT_WIRE_FORMAT_AND_PREFERENCE**](https://learn.microsoft.com/windows-hardware/drivers/ddi/d3dkmdt/ns-d3dkmdt-_d3dkmdt_wire_format_and_preference) value which indicates the wire format to be set for the path. The Preference field is reserved in this context so should be ignored by the driver. In the remaining five bit-fields, the OS will set one of the thirty bits to indicate which color encoding and at which bit depth the link should be driven.
...
Indicates what style of synchronization the OS is requesting. For a WDDM 2.4 - WDDM 2.6 driver, this value is set to [DXGK_SYNC_LOCK_STYLE_IDENTICAL](https://learn.microsoft.com/windows-hardware/drivers/ddi/d3dkmddi/ne-d3dkmddi-_dxgk_sync_lock_style) if sync-able displays are enabled and DXGK_SYNC_LOCK_STYLE_NONE otherwise.
