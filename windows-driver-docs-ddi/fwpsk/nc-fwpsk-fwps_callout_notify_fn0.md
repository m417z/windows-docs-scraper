...
*notifyFn0* callout function to notify the callout driver about events that are associated with the
callout.

**Note** *notifyFn0* is the specific version of [notifyFn](https://learn.microsoft.com/windows-hardware/drivers/ddi/_netvista/) used in Windows Vista and later. See [WFP Version-Independent Names and Targeting Specific Versions of Windows](https://learn.microsoft.com/windows/desktop/FWP/wfp-version-independent-names-and-targeting-specific-versions-of-windows) for more information. For Windows 8, [notifyFn2](https://learn.microsoft.com/windows-hardware/drivers/ddi/fwpsk/nc-fwpsk-fwps_callout_notify_fn2) is available. For Windows 7, [notifyFn1](https://learn.microsoft.com/windows-hardware/drivers/ddi/fwpsk/nc-fwpsk-fwps_callout_notify_fn1) is available.
...
A callout's
*notifyFn0* callout function can set the
...
[classifyFn0](https://learn.microsoft.com/windows-hardware/drivers/ddi/fwpsk/nc-fwpsk-fwps_callout_classify_fn0) callout function to preserve
...
A callout's
*notifyFn0* callout function can clean up any context associated with the filter when the filter is
deleted from the filter engine.

## Return value

A callout's
*notifyFn0* function returns one of the following NTSTATUS codes.
...
*notifyFn0* callout function does not recognize the type of notification that is passed in the
*notifyType* parameter, it ignores the notification and return STATUS_SUCCESS.
...
callout driver's
*notifyFn0* callout function to notify the callout about any of the existing filters. The filter
engine calls the callout driver's
*notifyFn0* callout function to notify the callout when new filters that specify the callout for the
filter's action are added to the filter engine. In this situation, a callout's
*notifyFn0* callout function might not get called for every filter in the filter engine that
...
*notifyFn0* callout function and passes FWP_CALLOUT_NOTIFY_DELETE_FILTER in the
