...
The **ReadNtStatusFromMode** function safely reads an NTSTATUS value from memory based on the specified processor mode.

## Parameters

### `Source`

[in] A pointer to the memory location from which to read the NTSTATUS value.
...
**ReadNtStatusFromMode** returns the NTSTATUS value read from the specified memory address.

## Remarks

This function provides a safe way to read an NTSTATUS value from memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
...
[**ReadNtStatusFromUser**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-readntstatusfromuser)

[**WriteNtStatusToMode**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-writentstatustomode)