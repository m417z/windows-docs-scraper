...
The **ReadNtStatusFromUserAcquire** function safely reads an NTSTATUS value from user-mode memory with [acquire semantics](https://learn.microsoft.com/windows-hardware/drivers/kernel/acquire-and-release-semantics).
...
This function provides a safe way to read an NTSTATUS value from user-mode memory. It ensures the provided address is a user-mode address before reading it. The [acquire semantics](https://learn.microsoft.com/windows-hardware/drivers/kernel/acquire-and-release-semantics) ensure that subsequent memory operations can't be reordered before this read operation.
...
[**WriteNtStatusToUserRelease**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-writentstatustouserrelease)