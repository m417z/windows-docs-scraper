...
The **ReadUIntPtrFromMode** function safely reads a UINT_PTR value from memory based on the specified processor mode.

## Parameters

### `Source`

[in] A pointer to the memory location from which to read the UINT_PTR value.
...
**ReadUIntPtrFromMode** returns the UINT_PTR value read from the specified memory address.

## Remarks

This function provides a safe way to read a UINT_PTR value from memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
...
[**ReadUIntPtrFromUser**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-readuintptrfromuser)

[**WriteUIntPtrToMode**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-writeuintptrtomode)