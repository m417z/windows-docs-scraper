...
The **WriteUIntPtrToMode** function safely writes a UINT_PTR value to memory based on the specified processor mode.

## Parameters

### `Destination`

[out] A pointer to the memory location where the UINT_PTR value will be written.
...
This function provides a safe way to write a UINT_PTR value to memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
