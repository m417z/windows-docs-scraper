...
The **ReadShortFromMode** function safely reads a SHORT value from memory based on the specified processor mode.

## Parameters

### `Source`

[in] A pointer to the memory location from which to read the SHORT value.
...
**ReadShortFromMode** returns the SHORT value read from the specified memory address.

## Remarks

This function provides a safe way to read a SHORT value from memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
...
[**WriteShortToMode**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-writeshorttomode)