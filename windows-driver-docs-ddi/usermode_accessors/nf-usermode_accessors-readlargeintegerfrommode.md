...
The **ReadLargeIntegerFromMode** function safely reads a LARGE_INTEGER value from memory based on the specified processor mode.

## Parameters

### `Source`

[in] A pointer to the memory location from which to read the LARGE_INTEGER value.
...
**ReadLargeIntegerFromMode** returns the LARGE_INTEGER value read from the specified memory address.

## Remarks

This function provides a safe way to read a LARGE_INTEGER value from memory, with extra safety checks when accessing user-mode memory. It ensures that the provided address is valid and accessible based on the specified processor mode.
...
[**ReadLargeIntegerFromUser**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-readlargeintegerfromuser)

[**WriteLargeIntegerToMode**](https://learn.microsoft.com/windows-hardware/drivers/ddi/usermode_accessors/nf-usermode_accessors-writelargeintegertomode)