...
A formatted [**WWAN_SAR_TRANSMISSION_STATUS_INFO**](https://learn.microsoft.com/windows-hardware/drivers/ddi/wwan/ns-wwan-_wwan_sar_transmission_status_info) structure that describes the modem's SAR transmission status.

## Remarks

This structure is used in the [NDIS_STATUS_WWAN_SAR_TRANSMISSION_STATUS](https://learn.microsoft.com/windows-hardware/drivers/network/ndis-status-wwan-sar-transmission-status) status notification in response to a previous [OID_WWAN_SAR_TRANSMISSION_STATUS](https://learn.microsoft.com/windows-hardware/drivers/network/oid-wwan-sar-transmission-status) Query or Set request.
