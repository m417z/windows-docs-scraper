...
The **STOR_POFX_DEVICE_V3** structure describes the power attributes of a storage device to the power management framework (PoFx). This structure is similar to [**STOR_POFX_DEVICE**](https://learn.microsoft.com/windows-hardware/drivers/ddi/storport/ns-storport-_stor_pofx_device) but allows the caller to specify an idle timeout value.
