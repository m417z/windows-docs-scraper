...
[in] Specifies an enumerator value of type [**STOR_SPINLOCK**](https://learn.microsoft.com/windows-hardware/drivers/ddi/storport/ne-storport-_stor_spinlock) that specifies the spin lock to acquire.
...
[in/out] A pointer to a buffer that, on return, will contain a lock handle. To release the lock, the caller must pass this handle to the [**StorPortReleaseSpinLock**](https://learn.microsoft.com/windows-hardware/drivers/ddi/storport/nf-storport-storportreleasespinlock) routine.
