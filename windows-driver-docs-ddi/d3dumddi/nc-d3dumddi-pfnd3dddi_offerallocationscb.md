...
A pointer to a [D3DDDICB_OFFERALLOCATIONS](https://learn.microsoft.com/windows-hardware/drivers/ddi/d3dumddi/ns-d3dumddi-_d3dddicb_offerallocations) structure that defines the video memory allocations that the driver offers.
...
After the driver calls **pfnOfferAllocationsCb** to offer an allocation to reuse, it must call [pfnReclaimAllocationsCb](https://learn.microsoft.com/windows-hardware/drivers/ddi/d3dumddi/nc-d3dumddi-pfnd3dddi_reclaimallocationscb) before it locks the allocation or submits it for rendering operations.
