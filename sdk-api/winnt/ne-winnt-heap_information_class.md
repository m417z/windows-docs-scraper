...
The heap features that are enabled. The available features vary based on operating system. Depending on the *HeapInformation* parameter in the [HeapQueryInformation](https://learn.microsoft.com/windows/desktop/api/heapapi/nf-heapapi-heapqueryinformation) or [HeapSetInformation](https://learn.microsoft.com/windows/desktop/api/heapapi/nf-heapapi-heapsetinformation) functions, specifying this enumeration value can indicate one of the following features:

* A standard heap that does not support look-aside lists.
* A heap that supports look-aside lists.
* A [low-fragmentation heap](https://learn.microsoft.com/windows/desktop/Memory/low-fragmentation-heap) (LFH), which does not support look-aside lists.
...
To retrieve information about a heap, use the [HeapQueryInformation](https://learn.microsoft.com/windows/desktop/api/heapapi/nf-heapapi-heapqueryinformation) function. To enable features for a heap, use the
