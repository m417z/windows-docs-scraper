...
After a navigation and view creation, either [IExplorerBrowserEvents::OnNavigationComplete](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/nf-shobjidl_core-iexplorerbrowserevents-onnavigationcomplete) or **IExplorerBrowserEvents::OnNavigationFailed** is called, depending on whether the destination could be navigated to. For example, a failure to navigate includes a destination that is not reached either because there is no route to the path or the user has canceled.