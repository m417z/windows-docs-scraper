...
Contains extended information about a shortcut menu command. This structure is an extended version of [CMINVOKECOMMANDINFO](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/ns-shobjidl_core-cminvokecommandinfo) that allows the use of Unicode values.
...
The size of this structure, in bytes. This member should be filled in by callers of [IContextMenu::InvokeCommand](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/nf-shobjidl_core-icontextmenu-invokecommand) and tested by the implementations to know that the structure is a **CMINVOKECOMMANDINFOEX** structure rather than [CMINVOKECOMMANDINFO](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/ns-shobjidl_core-cminvokecommandinfo).
...
A handle to the window that is the owner of the shortcut menu. An extension can also use this handle as the owner of any message boxes or dialog boxes it displays. Callers must specify a legitimate HWND that can be used as the owner window for any UI that may be displayed. Failing to specify an HWND when calling from a UI thread (one with windows already created) will result in reentrancy and possible bugs in the implementation of a [IContextMenu::InvokeCommand](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/nf-shobjidl_core-icontextmenu-invokecommand) call.
...
**fMask** member does not specify **CMIC_MASK_HOTKEY**, this member is ignored.
...
Although the [IContextMenu::InvokeCommand](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/nf-shobjidl_core-icontextmenu-invokecommand) declaration specifies a [CMINVOKECOMMANDINFO](https://learn.microsoft.com/windows/desktop/api/shobjidl_core/ns-shobjidl_core-cminvokecommandinfo) structure for the *pici* parameter, it can also accept a **CMINVOKECOMMANDINFOEX** structure. If you are implementing this method, you must inspect **cbSize** to determine which structure has been passed.
