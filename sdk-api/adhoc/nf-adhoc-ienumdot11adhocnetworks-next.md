...
A pointer to the first element in an array of [IDot11AdHocNetwork](https://learn.microsoft.com/windows/desktop/api/adhoc/nn-adhoc-idot11adhocnetwork) interfaces. The array is of size *cElt*. The array must exist and be of size *cElt* (at a minimum) before the **Next** method is called, although the array need not be initialized. Upon return, the previously existing array will contain pointers to **IDot11AdHocNetwork** objects.
