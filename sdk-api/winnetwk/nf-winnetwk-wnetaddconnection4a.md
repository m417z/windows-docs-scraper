...
The **WNetAddConnection4** function is similar to the [WNetAddConnection3](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnection3a) function. The main difference is that **WNetAddConnection4** has an additional parameter that allows the caller to specify an extendable property bag of parameters. The [WNetAddConnection2](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnection2a), **WNetAddConnection3**, and **WNetAddConnection4** functions supersede the [WNetAddConnection](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnectiona) function.
...
The *hwndOwner* parameter can be **NULL**. If it is, a call to **WNetAddConnection4** is equivalent to calling the [WNetAddConnection2](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnection2a) function.

### `lpNetResource`

A pointer to a [NETRESOURCE](https://learn.microsoft.com/windows/win32/api/winnetwk/ns-winnetwk-netresourcea) structure that specifies details of the proposed connection, such as information about the network resource, the local device, and the network resource provider.

You must specify the following members of the [NETRESOURCE](https://learn.microsoft.com/windows/win32/api/winnetwk/ns-winnetwk-netresourcea) structure:
...
The **WNetAddConnection4** function ignores the other members of the [NETRESOURCE](https://learn.microsoft.com/windows/win32/api/winnetwk/ns-winnetwk-netresourcea) structure.
...
If the function fails, the return value is a [system error code](https://learn.microsoft.com/windows/win32/Debug/system-error-codes), such as one of the following values:
...
| **ERROR_EXTENDED_ERROR** | A network-specific error occurred. Call the [WNetGetLastError](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetgetlasterrora) function to obtain a description of the error. |
...
The [WNetUseConnection](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetuseconnectiona) function is similar to the **WNetAddConnection4** function. The main difference is that **WNetUseConnection** can automatically select an unused local device to redirect to the network resource.

On Windows Server 2003 and Windows XP, the WNet functions create and delete network drive letters in the MS-DOS device namespace associated with a logon session because MS-DOS devices are identified by AuthenticationID (a [locally unique identifier](https://learn.microsoft.com/windows/win32/SecGloss/l-gly), or LUID, associated with a logon session.) This can affect applications that call one of the WNet functions to create a network drive letter under one user logon, but query for existing network drive letters under a different user logon. An example of this situation could be when a user's second logon is created within a logon session, for example, by calling the [CreateProcessAsUser](https://learn.microsoft.com/windows/win32/api/processthreadsapi/nf-processthreadsapi-createprocessasusera) function, and the second logon runs an application that calls the [GetLogicalDrives](https://learn.microsoft.com/windows/win32/api/fileapi/nf-fileapi-getlogicaldrives) function. The call to the **GetLogicalDrives** function does not return network drive letters created by WNet function calls under the first logon. Note that in the preceding example the first logon session still exists, and the example could apply to any logon session, including a Terminal Services session. For more information, see [Defining an MS-DOS Device Name](https://learn.microsoft.com/windows/win32/FileIO/defining-an-ms-dos-device-name).
...
Other network providers may support the **lpRemoteName** member of the [NETRESOURCE](https://learn.microsoft.com/windows/win32/api/winnetwk/ns-winnetwk-netresourcea) structure pointed to by the *lpNetResource* parameter that contains an IPv4 or IPv6 address, but this is up to specific network provider.
...
- [WNetAddConnection](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnectiona)
- [WNetAddConnection2](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnection2a)
- [WNetAddConnection3](https://learn.microsoft.com/windows/win32/api/winnetwk/nf-winnetwk-wnetaddconnection3a)