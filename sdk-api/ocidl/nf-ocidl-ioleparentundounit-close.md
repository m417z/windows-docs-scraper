...
An [IOleParentUndoUnit](https://learn.microsoft.com/windows/desktop/api/ocidl/nn-ocidl-ioleparentundounit) pointer to the currently open parent unit to be closed.
...
If it does have a child unit open, it calls the **IOleParentUndoUnit::Close** method on the child. If the child returns S_FALSE, then the parent undo unit verifies that *pPUU* points to the child unit, and closes that child undo unit. If the child returns S_OK then it handled the close internally and its parent should do nothing.
