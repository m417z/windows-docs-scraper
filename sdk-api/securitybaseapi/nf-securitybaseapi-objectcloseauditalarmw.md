...
A unique value representing the client's handle to the object. This should be the same value that was passed to the [AccessCheckAndAuditAlarm](https://learn.microsoft.com/windows/desktop/api/securitybaseapi/nf-securitybaseapi-accesscheckandauditalarmw) or [ObjectOpenAuditAlarm](https://learn.microsoft.com/windows/desktop/api/securitybaseapi/nf-securitybaseapi-objectopenauditalarmw) function.

### `GenerateOnClose` [in]

Specifies a flag set by a call to the [AccessCheckAndAuditAlarm](https://learn.microsoft.com/windows/desktop/api/securitybaseapi/nf-securitybaseapi-accesscheckandauditalarmw) or **ObjectCloseAuditAlarm** function when the object handle is created. If this flag is **TRUE**, the function generates an audit message. If it is **FALSE**, the function does not generate an audit message.
