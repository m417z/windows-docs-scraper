...
By default, queries do not receive updates after the current state of the system has been processed. Once all matching objects have been added to the result-set, the query enters the [DevQueryStateEnumCompleted](https://learn.microsoft.com/windows/win32/api/devquerydef/ne-devquerydef-dev_query_state) state and no further callbacks
...
A query may be aborted. This happens when an internal error happens that makes it impossible to communicate a notification to the client. For example, this can happen when the system is in an out-of-memory condition. When this happens, the caller’s result-set no longer reflects a true representation of the state of the query. The caller will receive a [DevQueryStateAborted](https://learn.microsoft.com/windows/win32/api/devquerydef/ne-devquerydef-dev_query_state) callback. No further callbacks will be received. The caller must close the query and create a new query to recover from this condition.
...
[DevCreateObjectQueryEx](https://learn.microsoft.com/windows/win32/api/devquery/nf-devquery-devcreateobjectqueryex)