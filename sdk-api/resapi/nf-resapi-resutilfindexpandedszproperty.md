...
Locates an expanded string property value in a [property list](https://learn.microsoft.com/previous-versions/windows/desktop/mscs/property-lists). The **PRESUTIL_FIND_EXPANDED_SZ_PROPERTY** type defines a pointer to this function.
...
### `pszPropertyValue` [out, optional]

Pointer to a **WCHAR** pointer to a buffer (allocated by the function) containing a copy of the property value. You must call [LocalFree](https://learn.microsoft.com/windows/desktop/api/winbase/nf-winbase-localfree) (on **pszPropertyValue*) to free the allocated memory. If no value is required, pass **NULL** for this parameter.
...
If [ResUtilFindExpandSzProperty](https://learn.microsoft.com/windows/desktop/api/resapi/nf-resapi-resutilfindexpandszproperty) is successful, **pszPropertyValue* points to a copy of the data stored in *pPropertyList*. Be sure to call [LocalFree](https://learn.microsoft.com/windows/desktop/api/winbase/nf-winbase-localfree) on **pszPropertyValue* to prevent memory leaks.
