...
Called by the [Local Security Authority](https://learn.microsoft.com/windows/desktop/SecGloss/l-gly) (LSA) when a logon application with a trusted connection to the LSA calls the
[LsaCallAuthenticationPackage](https://learn.microsoft.com/windows/desktop/api/ntsecapi/nf-ntsecapi-lsacallauthenticationpackage) function and specifies the authentication package's identifier.

**LsaApCallPackage** is called for logon applications only; calls from applications that do not have the SeTcbPrivilege privilege are routed to the specified authentication package's
[LsaApCallPackageUntrusted](https://learn.microsoft.com/previous-versions/windows/desktop/legacy/aa378218(v=vs.85)) function instead.
...
[AllocateClientBuffer](https://learn.microsoft.com/windows/desktop/api/ntsecpkg/nc-ntsecpkg-lsa_allocate_client_buffer) function to allocate the buffer within the client process. The contents of this buffer are specific to the authentication package.
...
[LsaApCallPackageUntrusted](https://learn.microsoft.com/previous-versions/windows/desktop/legacy/aa378218(v=vs.85))
