...
[LSA_CLIENT_REQUEST](https://learn.microsoft.com/windows/desktop/SecAuthN/plsa-client-request) buffer that represents the LSA client's request. Your authentication package can pass this value into
[AllocateClientBuffer](https://learn.microsoft.com/windows/desktop/api/ntsecpkg/nc-ntsecpkg-lsa_allocate_client_buffer) and
[FreeClientBuffer](https://learn.microsoft.com/windows/desktop/api/ntsecpkg/nc-ntsecpkg-lsa_free_client_buffer) in order to identify the client process in which memory should be allocated or freed.

### `LogonType` [in]

A
[SECURITY_LOGON_TYPE](https://learn.microsoft.com/windows/desktop/api/ntsecapi/ne-ntsecapi-security_logon_type) value identifying the type of logon requested.
...
[AllocateClientBuffer](https://learn.microsoft.com/windows/desktop/api/ntsecpkg/nc-ntsecpkg-lsa_allocate_client_buffer) function. However, if the LSA subsequently encounters an error that prevents a successful logon, the LSA will free this buffer.
...
Pointer to an
[LUID](https://learn.microsoft.com/windows/desktop/api/winnt/ns-winnt-luid) that receives the new logon ID that uniquely identifies this logon session. The authentication package is responsible for allocating this **LUID**, and creating the logon session for this logon.
...
[LSA_TOKEN_INFORMATION_TYPE](https://learn.microsoft.com/windows/desktop/api/ntsecpkg/ne-ntsecpkg-lsa_token_information_type) value that indicates the type of information returned for inclusion in the token to be created. The information is returned in the *TokenInformation* buffer.
...
| **STATUS_ACCOUNT_RESTRICTION** | The user account and password were legitimate, but user account restrictions prevent logon at this time. For additional information, see the *SubStatus* parameter. |
| **STATUS_BAD_VALIDATION_CLASS** | The authentication information provided is not recognized by the specified authentication package. |
...
Authentication packages must implement one of the following functions: **LsaApLogonUser**,
