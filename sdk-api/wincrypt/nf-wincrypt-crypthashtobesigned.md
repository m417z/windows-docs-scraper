...
The **CryptHashToBeSigned** function computes the [hash](https://learn.microsoft.com/windows/desktop/SecGloss/h-gly) of the encoded content from a signed and encoded [certificate](https://learn.microsoft.com/windows/desktop/SecGloss/c-gly). The hash is performed on only the "to be signed" encoded content and its signature.
...
[CryptHashData](https://learn.microsoft.com/windows/desktop/api/wincrypt/nf-wincrypt-crypthashdata) might be propagated to this function. This function has the following error codes.
...
| **NTE_BAD_ALGID** | The [object identifier](https://learn.microsoft.com/windows/desktop/SecGloss/o-gly) (OID) of the signature algorithm does not map to a known or supported hash algorithm. |
