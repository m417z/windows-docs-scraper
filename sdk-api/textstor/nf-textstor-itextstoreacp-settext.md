...
Applications should start a composition by first using [ITextStoreACP::InsertTextAtSelection](https://learn.microsoft.com/windows/desktop/api/textstor/nf-textstor-itextstoreacp-inserttextatselection). **ITextStoreACP::SetText** should be used only within an existing composition. If there is no active composition at the time **SetText** is called, the TSF manager creates a composition that lasts just long enough to wrap the call to **SetText**.
...
This method should call the [ITextStoreACP::SetSelection](https://learn.microsoft.com/windows/desktop/api/textstor/nf-textstor-itextstoreacp-setselection) method to select the text to be changed. After successfully executing the **ITextStoreACP::SetSelection** method, this method then calls the **ITextStoreACP::InsertTextAtSelection** method to perform the actual text change.
