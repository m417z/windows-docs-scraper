...
The
**PMGM_LOCAL_LEAVE_CALLBACK** callback is a call into a routing protocol to notify the routing protocol that the IGMP has detected that it no longer has receivers for a group on an interface that is currently owned by the routing protocol.

This callback is invoked when the
[MgmDeleteGroupMembershipEntry](https://learn.microsoft.com/windows/desktop/api/mgm/nf-mgm-mgmdeletegroupmembershipentry) function is called by IGMP.
...
[PMGM_LOCAL_JOIN_CALLBACK](https://learn.microsoft.com/windows/desktop/api/mgm/nc-mgm-pmgm_local_join_callback)