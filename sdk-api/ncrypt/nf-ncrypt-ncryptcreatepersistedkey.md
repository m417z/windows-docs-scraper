...
The **NCryptCreatePersistedKey** function creates a new key and stores it in the specified key storage provider. After you create a key by using this function, you can use the [NCryptSetProperty](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptsetproperty) function to set its properties; however, the key cannot be used until the [NCryptFinalizeKey](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptfinalizekey) function is called.

## Parameters

### `hProvider` [in]

The handle of the key storage provider to create the key in. This handle is obtained by using the [NCryptOpenStorageProvider](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptopenstorageprovider) function.

### `phKey` [out]

The address of an **NCRYPT_KEY_HANDLE** variable that receives the handle of the key. When you have finished using this handle, release it by passing it to the [NCryptFreeObject](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptfreeobject) function. To delete the key file on disk, pass the handle to the [NCryptDeleteKey](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptdeletekey) function. This will also release the handle. So applications may pass the handle to either **NCryptFreeObject** or **NCryptDeleteKey**, but not both.

### `pszAlgId` [in]

A pointer to a null-terminated Unicode string that contains the identifier of the cryptographic algorithm to create the key. This can be one of the standard [CNG Algorithm Identifiers](https://learn.microsoft.com/windows/win32/SecCNG/cng-algorithm-identifiers) or the identifier for another registered algorithm.
...
If you are creating an RSA key pair, you can also have the key stored in legacy storage so that it can be used with the CryptoAPI by passing the **NCRYPT_WRITE_KEY_TO_LEGACY_STORE_FLAG** flag to the [NCryptFinalizeKey](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptfinalizekey) function when the key is finalized.
...
[NCryptDeleteKey](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptdeletekey)

[NCryptFinalizeKey](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptfinalizekey)

[NCryptImportKey](https://learn.microsoft.com/windows/win32/api/ncrypt/nf-ncrypt-ncryptimportkey)