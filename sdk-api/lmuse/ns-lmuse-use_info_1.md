...
The
**USE_INFO_1** structure contains information about the connection between a local device and a shared resource. The information includes connection status and connection type.
...
| Value | Meaning |
| --- | --- |
| **USE_WILDCARD** | Matches the type of the server's shared resources. Wildcards can be used only with the [NetUseAdd](https://learn.microsoft.com/windows/desktop/api/lmuse/nf-lmuse-netuseadd) function, and only when the **ui1_local** member is **NULL**. For more information, see the following Remarks section. |
...
[NetUseAdd](https://learn.microsoft.com/windows/desktop/api/lmuse/nf-lmuse-netuseadd) function.
...
Specifying a **ui1_local** member that is **NULL** requests authentication with the server without redirecting a drive letter or a device. Future redirections involving the server while the same connection is in effect use the password specified by the **ui1_password** member in the initial call to the
[NetUseAdd](https://learn.microsoft.com/windows/desktop/api/lmuse/nf-lmuse-netuseadd) function.
