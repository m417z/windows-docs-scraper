...
| Value | Meaning |
| --- | --- |
| ACM_FILTERTAGDETAILSF_FILTERTAG |[ACMFILTERTAGDETAILS]() structure. The filter tag details will be returned in the structure pointed to by *paftd*. If an application specifies an ACM driver handle for *had*, details on the filter tag will be returned for that driver. If an application specifies **NULL** for *had*, the ACM finds the first acceptable driver to return the details.
