...
When the **Push** method is called, the data is being sent to the provider of the pipe. The caller fills the buffer with the data and then calls **Push**. The number of doubles being sent is specified in the *cSent* parameter. The caller is responsible for ensuring that the buffer is valid for the duration of the call.
