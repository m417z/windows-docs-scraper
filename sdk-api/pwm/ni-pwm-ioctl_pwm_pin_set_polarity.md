...
Sets the signal polarity of the pin or channel. The control code sets the signal polarity based on a [PWM_PIN_SET_POLARITY_INPUT](https://learn.microsoft.com/windows/desktop/api/pwm/ns-pwm-pwm_pin_set_polarity_input) structure. The signal polarity is either Active High or Active Low, as defined in the [PWM_POLARITY](https://learn.microsoft.com/windows/desktop/api/pwm/ne-pwm-pwm_polarity) enumeration.
...
| *dwIoControlCode* [in] | The control code for the operation. Use **IOCTL_PWM_PIN_SET_POLARITY** for this operation. |
| *lpInBuffer* | A pointer to a buffer that contains a [PWM_PIN_GET_POLARITY_OUTPUT](https://learn.microsoft.com/windows/desktop/DevIO/pwm-pin-get-polarity-output) structure. This represents the polarity value of the PWM controller, and is either Active High or Active Low. |
...
Changing polarity is only allowed when the pin is stopped. You can tell whether the pin is stopped by using the [IOCTL_PWM_PIN_IS_STARTED](https://learn.microsoft.com/windows/desktop/api/pwm/ni-pwm-ioctl_pwm_pin_is_started) control code.
