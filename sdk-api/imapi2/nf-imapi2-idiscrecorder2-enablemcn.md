...
MCN is the CD-ROM device driver's method of detecting media change and state changes in the CD-ROM device. For example, when you change the media in a CD-ROM device, a MCN message is sent to trigger media features, such as Autoplay. MCN is enabled by default. Call this method to enable notifications when the notifications have been disabled using [IDiscRecorder2::DisableMcn](https://learn.microsoft.com/windows/desktop/api/imapi2/nf-imapi2-idiscrecorder2-disablemcn).

Note that [DisableMcn](https://learn.microsoft.com/windows/desktop/api/imapi2/nf-imapi2-idiscrecorder2-disablemcn) increments a reference count each time it is called. The EnableMcn method decrements the count. The device is enabled when the reference count is zero.
