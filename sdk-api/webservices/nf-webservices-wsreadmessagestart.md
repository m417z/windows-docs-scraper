...
This function is like [WsReadEnvelopeStart](https://learn.microsoft.com/windows/desktop/api/webservices/nf-webservices-wsreadenvelopestart) but is used with channels.

Upon success, the headers will be stored in the message and can be accessed in a random-access
fashion (for example, using [WsGetHeader](https://learn.microsoft.com/windows/desktop/api/webservices/nf-webservices-wsgetheader)).

The message must be in [WS_MESSAGE_STATE_EMPTY](https://learn.microsoft.com/windows/desktop/api/webservices/ne-webservices-ws_message_state) state. Upon success,
the message will have transitioned to **WS_MESSAGE_STATE_READING** state.

To deserialize an element of the message body, use [WsReadBody](https://learn.microsoft.com/windows/desktop/api/webservices/nf-webservices-wsreadbody). To read
directly from the XML Reader of the message, first get the reader using
the [WS_MESSAGE_PROPERTY_BODY_READER](https://learn.microsoft.com/windows/desktop/api/webservices/ne-webservices-ws_message_property_id) property.

If the channel input is streamed ([WS_STREAMED_INPUT_TRANSFER_MODE](https://learn.microsoft.com/windows/desktop/api/webservices/ne-webservices-ws_transfer_mode)), then
[WsFillBody](https://learn.microsoft.com/windows/desktop/api/webservices/nf-webservices-wsfillbody) must be called to receive the next part of the body before
it is actually read.