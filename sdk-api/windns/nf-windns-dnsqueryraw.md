...
Queries are performed asynchronously; and results are passed to a [DNS_QUERY_RAW_COMPLETION_ROUTINE](https://learn.microsoft.com/windows/win32/api/windns/nc-windns-dns_query_raw_completion_routine) asynchronous callback function that you implement. To cancel a query, call [DnsCancelQueryRaw](https://learn.microsoft.com/windows/win32/api/windns/nf-windns-dnscancelqueryraw).

## Parameters

### `queryRequest`

Type: \_In\_ **[DNS_QUERY_RAW_REQUEST](https://learn.microsoft.com/windows/win32/api/windns/ns-windns-dns_query_raw_request)\***

The query request.

### `cancelHandle`

Type: \_Inout\_ **[DNS_QUERY_RAW_CANCEL](https://learn.microsoft.com/windows/win32/api/windns/ns-windns-dns_query_raw_cancel)\***

Used to obtain a cancel handle, which you can pass to [DnsCancelQueryRaw](https://learn.microsoft.com/windows/win32/api/windns/nf-windns-dnscancelqueryraw) should you need to cancel the query.

## Return value

A **DNS_STATUS** value indicating success or failure. If **DNS_REQUEST_PENDING** is returned, then when the query completes, the system calls the [DNS_QUERY_RAW_COMPLETION_ROUTINE](https://learn.microsoft.com/windows/win32/api/windns/nc-windns-dns_query_raw_completion_routine) implementation that you passed in the *queryCompletionCallback* member of *queryRequest*. That callback will received the results of the query if successful, or any failures or cancellations.
...
In this example, the query is initiated with a query name and type, but is then cancelled with [DnsCancelQueryRaw](https://learn.microsoft.com/windows/win32/api/windns/nf-windns-dnscancelqueryraw).
