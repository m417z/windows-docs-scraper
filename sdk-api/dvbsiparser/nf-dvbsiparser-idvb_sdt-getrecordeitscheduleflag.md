...
The **GetRecordEITScheduleFlag** method queries whether the current transport stream contains schedule EIT information for a particular service.
...
Pointer to a variable that receives a Boolean value. The value is **TRUE** if the EIT_schedule_flag bit is set, or **FALSE** otherwise. The value **TRUE** indicates that the current transport stream contains schedule EIT information for the service.
