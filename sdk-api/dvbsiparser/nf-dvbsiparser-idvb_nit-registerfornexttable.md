...
Handle to an event created by the caller. The object signals the event when the *next* table arrives. When the event is signaled, call the [IDVB_NIT::GetNextTable](https://learn.microsoft.com/previous-versions/windows/desktop/api/dvbsiparser/nf-dvbsiparser-idvb_nit-getnexttable) method to retrieve the table.
