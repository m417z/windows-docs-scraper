...
### `pRequestedOutputFormat` [in]

Output format to check for being supported.

### `ppSupportedOutputFormat` [out]

If not NULL and the output format is not supported for the given input format, *ppSupportedOutputFormat* returns a pointer to the closest output format that is supported. Use [XAPOFree](https://learn.microsoft.com/windows/desktop/api/xapo/nf-xapo-xapofree) to free the returned structure.
