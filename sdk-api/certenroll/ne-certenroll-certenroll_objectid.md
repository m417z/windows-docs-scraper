...
The **CERTENROLL_OBJECTID** enumeration type contains the predefined object identifiers (OIDs) supported by Certificate Enrollment API.
...
Contains an OID that specifies the type of the data being signed in a PKCS #7 message. Possible examples include **XCN_OID_RSA_signedData** or **XCN_OID_RSA_envelopedData**.
...
Contains the PKCS #12 display name attribute transmitted in the **SafeBag** data of a PKCS #12 PFX message. The attribute specifies the display name of the object with which it is associated.

### `XCN_OID_PKCS_12_LOCAL_KEY_ID:164`

Contains a PKCS #12 key identifier attribute transmitted in the **SafeBag** data of a PKCS #12 PFX message. The identifier is only used in local applications.
