...
| **MF_E_INVALIDREQUEST** | Cannot frame step. Reasons for this error code include:<br><br>* There is no media item queued for playback.<br>* The current media item does not contain video. |
...
| **MF_E_UNSUPPORTED_RATE** | The media source does not support frame stepping, or the current playback rate is negative. |

## Remarks

This method completes asynchronously. When the operation completes, the application's [IMFPMediaPlayerCallback::OnMediaPlayerEvent](https://learn.microsoft.com/windows/desktop/api/mfplay/nf-mfplay-imfpmediaplayercallback-onmediaplayerevent) callback method is invoked. The event type is **MFP_EVENT_TYPE_FRAME_STEP**.
