...
A pointer to a [WNDCLASSEX](https://learn.microsoft.com/windows/desktop/api/winuser/ns-winuser-wndclassexa) structure. You must fill the structure with the appropriate class attributes before passing it to the function.
...
**RegisterClassExA**, the application tells the system that the windows of the created class expect messages with text or character parameters to use the ANSI character set; if you register it by using
**RegisterClassExW**, the application requests that the system pass text parameters of messages as Unicode. The [IsWindowUnicode](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-iswindowunicode) function enables applications to query the nature of each window. For more information on ANSI and Unicode functions, see [Conventions for Function Prototypes](https://learn.microsoft.com/windows/desktop/Intl/conventions-for-function-prototypes).
