...
To use extended window styles in addition to the styles supported by **CreateWindow**, use the [CreateWindowEx](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-createwindowexw) function.
...
A **null**-terminated string or a class atom created by a previous call to the [RegisterClass](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-registerclassw) or [RegisterClassEx](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-registerclassexw) function. The atom must be in the low-order word of *lpClassName*; the high-order word must be zero. If *lpClassName* is a string, it specifies the window class name. The class name can be any name registered with **RegisterClass** or **RegisterClassEx**, provided that the module that registers the class is also the module that creates the window. The class name can also be any of the predefined system class names. For a list of system class names, see the Remarks section.
...
A pointer to a value to be passed to the window through the [CREATESTRUCT](https://learn.microsoft.com/windows/desktop/api/winuser/ns-winuser-createstructw) structure (**lpCreateParams** member) pointed to by the *lParam* param of the [WM_CREATE](https://learn.microsoft.com/windows/desktop/winmsg/wm-create) message. This message is sent to the created window by this function before it returns.

If an application calls **CreateWindow** to create a MDI client window, *lpParam* should point to a [CLIENTCREATESTRUCT](https://learn.microsoft.com/windows/desktop/api/winuser/ns-winuser-clientcreatestruct) structure. If an MDI client window calls **CreateWindow** to create an MDI child window, *lpParam* should point to a [MDICREATESTRUCT](https://learn.microsoft.com/windows/desktop/api/winuser/ns-winuser-mdicreatestructw) structure. *lpParam* may be **NULL** if no additional data is needed.
...
Before returning, **CreateWindow** sends a [WM_CREATE](https://learn.microsoft.com/windows/desktop/winmsg/wm-create) message to the window procedure. For overlapped, pop-up, and child windows, **CreateWindow** sends **WM_CREATE**, [WM_GETMINMAXINFO](https://learn.microsoft.com/windows/desktop/winmsg/wm-getminmaxinfo), and [WM_NCCREATE](https://learn.microsoft.com/windows/desktop/winmsg/wm-nccreate) messages to the window. The
*lParam* parameter of the **WM_CREATE** message contains a pointer to a [CREATESTRUCT](https://learn.microsoft.com/windows/desktop/api/winuser/ns-winuser-createstructw) structure. If the **WS_VISIBLE** style is specified, **CreateWindow** sends the window all the messages required to activate and show the window.
...
**CreateWindow** is implemented as a call to the [CreateWindowEx](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-createwindowexw) function, as shown below.
...
[Common Control Window Classes](https://learn.microsoft.com/windows/desktop/Controls/common-control-window-classes)

**Conceptual**

[CreateWindowEx](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-createwindowexw)
...
[RegisterClass](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-registerclassw)

[RegisterClassEx](https://learn.microsoft.com/windows/desktop/api/winuser/nf-winuser-registerclassexw)
