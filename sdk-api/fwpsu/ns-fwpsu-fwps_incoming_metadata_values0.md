...
> [!NOTE]
> **FWPS_INCOMING_METADATA_VALUES0** is a specific version of **FWPS_INCOMING_METADATA_VALUES**. For more info, see [WFP version-independent names and targeting specific versions of Windows](https://learn.microsoft.com/windows/win32/fwp/wfp-version-independent-names-and-targeting-specific-versions-of-windows).
...
A pointer to an [FWP_BYTE_BLOB](https://learn.microsoft.com/windows/win32/api/fwptypes/ns-fwptypes-fwp_byte_blob) structure that contains the full path to the process that owns the endpoint. This member contains valid data only if the **FWPS_METADATA_FIELD_PROCESS_PATH** flag is set in the *currentMetadataValues* member.
