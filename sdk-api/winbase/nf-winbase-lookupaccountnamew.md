...
A pointer to a buffer that receives the name of the domain where the account name is found. For computers that are not joined to a domain, this buffer receives the computer name. If this parameter is **NULL**, *cchReferencedDomainName* must be zero.

### `cchReferencedDomainName` [in, out]

A pointer to a variable. On input, this value specifies the size, in **TCHAR**s, of the *ReferencedDomainName* buffer. If the function fails because the buffer is too small, this variable receives the required buffer size, including the terminating **null** character.
