...
Supplies the XState compaction mask to use when allocating the *Context* structure.
...
When XState is enabled in compaction mode, specifying an *XStateCompactionMask* that contains only a subset of the enabled XState components can decrease the buffer size required to store the *Context*.
This is particularly useful if the system has many XState components enabled, but the *Context* will only be used to affect a small number of XState components.
The full set of enabled XState components can be obtained by calling [GetEnabledXStateFeatures](https://learn.microsoft.com/windows/win32/api/winbase/nf-winbase-getenabledxstatefeatures).
