...
Gets the current in-place state as specified by the [InPlaceState Enumeration](https://learn.microsoft.com/windows/win32/api/peninputpanel/ne-peninputpanel-inplacestate).
...
When the Tablet PC Input Panel is closed or hidden, the current in-place state is reset to the default state, unless the default state is [Auto](https://learn.microsoft.com/windows/win32/api/peninputpanel/ne-peninputpanel-inplacestate), in which case, the current in-place state is reset to **Hover**.
