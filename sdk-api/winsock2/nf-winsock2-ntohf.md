...
The
**ntohf** inline function converts an **unsigned __int32** from TCP/IP network order to host byte order (which is little-endian on Intel processors) and returns a **float**.
...
The
**ntohf** function returns the value supplied in the *value* parameter with the byte order reversed. If *value* is already in host byte order, then this function will reverse it. It is up to the application to determine if the byte order must be reversed.

## Remarks

The
**ntohf** inline function takes an **unsigned __int32** in TCP/IP network byte order (the AF_INET or AF_INET6 address family) and returns a **float** that contains a number in host byte order.

The
**ntohf** function can be used to convert an IPv4 address in network byte order to the IPv4 address in host byte order. This function does not do any checking to determine if the *value* parameter is a valid IPv4 address.

The **ntohf** function does not require that the Winsock DLL has previously been loaded with a successful
