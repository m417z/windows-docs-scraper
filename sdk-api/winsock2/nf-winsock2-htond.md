...
The
**htond** inline function converts a **double** from host to TCP/IP network byte order (which is big-endian).
...
The
**htond** function returns the value in TCP/IP's network byte order.

## Remarks

The
**htond** inline function takes a **double** that contains number in host byte order and returns a 64-bit unsigned number in the network byte order used in TCP/IP networks (the AF_INET or AF_INET6 address family).

The
**htond** inline function can be used to convert an IPv4 address in host byte order to the IPv4 address in network byte order. This function does not do any checking to determine if the *value* parameter is a valid IPv4 address.
