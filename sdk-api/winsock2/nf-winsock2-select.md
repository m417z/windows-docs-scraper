...
| **[WSAEINVAL](https://learn.microsoft.com/windows/desktop/WinSock/windows-sockets-error-codes-2)** | **WSAEINVAL** is returned if the *time-out* value is invalid, or if all three descriptor parameters are either **null** pointers or valid pointers to empty *fd_set* structures containing no sockets. |
