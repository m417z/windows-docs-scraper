...
[Windows Event Log](https://learn.microsoft.com/windows/win32/wes/windows-event-log).
...
- [Write an ETW manifest](https://learn.microsoft.com/windows/win32/wes/writing-an-instrumentation-manifest)
...
macro. The manifest does not need to include `` or ``
definitions since TraceLogging events are self-describing.
- In the build process for your component, use
[Message Compiler (MC.exe)](https://learn.microsoft.com/windows/win32/wes/message-compiler--mc-exe-) from
...
constants will be different for each channel.)
  - `ManifestName.rc`:
[Resource Compiler (RC.exe)](https://learn.microsoft.com/windows/win32/menurc/resource-compiler) script
...
_ChannelSymbol_KEYWORD_ constants.
  - If you specified the _symbol_ attribute on the `` or
`` definition in the manifest, the name of _ChannelSymbol_
...
from your `` element and the name from your `` or
`` element).
...
manifest
[must be included in the resources](https://learn.microsoft.com/windows/win32/wes/compiling-an-instrumentation-manifest)
...
[register your manifest](https://learn.microsoft.com/windows/win32/wes/developing-a-provider) on the
target system using
