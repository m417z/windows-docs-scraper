...
The Write Uncorrectable command that is used to mark a range of logical blocks as invalid. When the specified logical block(s) are read after this operation, a failure is returned with [Unrecovered Read Error status](https://learn.microsoft.com/windows/win32/api/nvme/ne-nvme-nvme_status_media_error_codes). To clear the invalid logical block status, a write operation is performed on those logical blocks.
...
NVM Commands are submitted by the host when the controller is ready as indicated in the [Controller Status](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_status) register and after the appropriate I/O Submission Queue(s) and I/O Completion Queue(s) have been created.
