...
A [NVME_CONTROLLER_CAPABILITIES](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_capabilities) structure that indicates the basic capabilities of the controller to host software.

The Controller Capabilities **CAP** register starts at Offset 00h.

### `VS`

a [NVME_VERSION](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_version) structure that indicates the major and minor version of the NVM Express specification that the controller implementation supports. Valid versions of the specification are: 1.0, 1.1, and 1.2.
...
A [NVME_CONTROLLER_CONFIGURATION](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_configuration) structure that contains read/write configuration settings for the controller.

Host software must set the Arbitration Mechanism (**AMS**), Memory Page Size (**MPS**), and Command Set (**CSS**) fields in [NVME_CONTROLLER_CONFIGURATION](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_configuration) to valid values prior to enabling the controller by setting the Enable (**EN**) field to `1`.
...
A [NVME_CONTROLLER_STATUS](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_status) structure that indicates controller status.

The Controller Status **CSTS** register starts at Offset 1Ch.

### `NSSR`

A [NVME_NVM_SUBSYSTEM_RESET](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_nvm_subsystem_reset) structure that provides host software with the capability to initiate an NVM Subsystem Reset.

Support for this optional register is indicated by the state of the NVM Subsystem Reset Supported (**NSSRS**) field in the [Controller Capabilities](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_capabilities). If the register is not supported, then the address range occupied by the register is reserved.

The (Optional) NVM Subsystem Reset register starts at Offset 20h.

### `AQA`

A [NVME_ADMIN_QUEUE_ATTRIBUTES](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_admin_queue_attributes) structure that specifies the Admin Queue Attributes for the Admin Submission Queue and Admin Completion Queue.

The Admin Queue Attributes **AQA** register starts at Offset 24h.

### `ASQ`

A [NVME_ADMIN_SUBMISSION_QUEUE_BASE_ADDRESS](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_admin_submission_queue_base_address) structure that specifies the base memory address of the Admin Submission Queue.

The Admin Submission Queue Base Address register starts at Offset 28h.

### `ACQ`

A [NVME_ADMIN_COMPLETION_QUEUE_BASE_ADDRESS](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_admin_completion_queue_base_address) structure that specifies the base memory address of the Admin Completion Queue.

Admin Completion Queue Base Address register starts at Offset 30h.

### `CMBLOC`

A [NVME_CONTROLLER_MEMORY_BUFFER_LOCATION](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_memory_buffer_location) structure that specifies the location of the Controller Memory Buffer.
...
A [NVME_CONTROLLER_MEMORY_BUFFER_SIZE](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_controller_memory_buffer_size) structure that specifies the size of the Controller Memory Buffer.
...
Specifies the start of the first Doorbell register. The Admin [Submission Queue Tail Doorbell](https://learn.microsoft.com/windows/win32/api/nvme/ns-nvme-nvme_submission_queue_tail_doorbell).
