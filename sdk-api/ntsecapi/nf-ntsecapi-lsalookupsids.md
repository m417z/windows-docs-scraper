...
[**LsaLookupSids** is available for use in the operating systems specified in the Requirements section. It may be altered or unavailable in subsequent versions. Instead, use [LsaLookupSids2](https://learn.microsoft.com/windows/desktop/api/ntsecapi/nf-ntsecapi-lsalookupsids2).]

The **LsaLookupSids** function looks up the names that correspond to an array of [security identifiers](https://learn.microsoft.com/windows/desktop/SecGloss/s-gly) (SIDs). If **LsaLookupSids** cannot find a name that corresponds to a SID, the function returns the SID in character form.
