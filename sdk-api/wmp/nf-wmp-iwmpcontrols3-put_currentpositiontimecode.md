...
The time code is in the format [*range*]*hh*:*mm*:*ss*.*ff* where [*range*] represents the range, *hh* represents hours, *mm* represents minutes, *ss* represents seconds, and *ff* represents frames. When specifying a value for **put_currentPositionTimecode**, you must include all eight digits using zeros as placeholders.
...
Specifying and retrieving values using **put_currentPositionTimecode** and **get_currentPositionTimecode** are supported only for files that contain SMPTE time code information.
...
[IWMPControls3::get_currentPositionTimecode](https://learn.microsoft.com/windows/desktop/api/wmp/nf-wmp-iwmpcontrols3-get_currentpositiontimecode)