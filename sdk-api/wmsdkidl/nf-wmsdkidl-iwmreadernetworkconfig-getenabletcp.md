...
Pointer to a variable that receives a Boolean value. If the value is **TRUE**, the reader object includes TCP when it performs protocol rollover. If the value is **FALSE**, the reader does not use TCP for protocol rollover. However, the reader will still use TCP if the URL explicitly specifies a TCP-based protocol, such as MMST or RTSPT.
