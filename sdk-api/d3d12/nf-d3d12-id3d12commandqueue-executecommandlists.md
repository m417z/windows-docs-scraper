...
The array of [ID3D12CommandList](https://learn.microsoft.com/windows/desktop/api/d3d12/nn-d3d12-id3d12commandlist) command lists to be executed.
...
Bundles can't be submitted to a command queue directly. If a bundle is passed to this method, the runtime will drop the call. The runtime will also drop the call if the [Close](https://learn.microsoft.com/windows/desktop/api/d3d12/nf-d3d12-id3d12graphicscommandlist-close) function has not been called on one or more of the command lists.

The runtime will detect if the command allocators associated with the command lists have been reset after [Close](https://learn.microsoft.com/windows/desktop/api/d3d12/nf-d3d12-id3d12graphicscommandlist-close) was called. The runtime will drop the call and remove the device in this situation.
