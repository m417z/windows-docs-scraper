...
**Note** This function is the same as [TransparentBlt](https://learn.microsoft.com/windows/desktop/api/wingdi/nf-wingdi-transparentblt).
...
The [GdiTransparentBlt](https://learn.microsoft.com/windows/desktop/api/wingdi/nf-wingdi-transparentblt) function supports all formats of source bitmaps. However, for 32 bpp bitmaps, it just copies the alpha value over. Use [AlphaBlend](https://learn.microsoft.com/windows/desktop/api/wingdi/nf-wingdi-alphablend) to specify 32 bits-per-pixel bitmaps with transparency.

If the source and destination rectangles are not the same size, the source bitmap is stretched to match the destination rectangle. When the [SetStretchBltMode](https://learn.microsoft.com/windows/desktop/api/wingdi/nf-wingdi-setstretchbltmode) function is used, the *iStretchMode* modes of BLACKONWHITE and WHITEONBLACK are converted to COLORONCOLOR for the **GdiTransparentBlt** function.
