...
The **LOGBRUSH32** structure defines the style, color, and pattern of a physical brush. It is similar to [LOGBRUSH](https://learn.microsoft.com/windows/desktop/api/wingdi/ns-wingdi-logbrush), but it is used to maintain compatibility between 32-bit platforms and 64-bit platforms when we record the metafile record on one platform and then play it on another. Thus, it is only used in [EMRCREATEBRUSHINDIRECT](https://learn.microsoft.com/windows/desktop/api/wingdi/ns-wingdi-emrcreatebrushindirect). If the code will only be on one platform, **LOGBRUSH** is sufficient.
...
[EMRCREATEBRUSHINDIRECT](https://learn.microsoft.com/windows/desktop/api/wingdi/ns-wingdi-emrcreatebrushindirect)
