...
A pointer to a caller-initialized [SP_DRVINFO_DATA](https://learn.microsoft.com/windows/desktop/api/setupapi/ns-setupapi-sp_drvinfo_data_v2_a) structure that receives information about the enumerated driver. The caller must set *DriverInfoData.***cbSize** to **sizeof(**SP_DRVINFO_DATA**)** before calling **SetupDiEnumDriverInfo**. If the **cbSize** member is not properly set, **SetupDiEnumDriverInfo** will return **FALSE**.
