...
The
**SetupAddInstallSectionToDiskSpaceList** function searches for **CopyFile** and **DelFile** lines in an **Install** section of an INF file. The function then adds the file operations specified in those sections to a disk-space list.
...
Handle to an open INF file that contains the **Install** section to be searched. If *ListInfHandle* is not specified, the INF file must also contain the section specified by *SectionName*.

### `LayoutInfHandle` [in]

This parameter, if specified, provides the handle to the INF file that contains the **SourceDisksFiles** sections. Otherwise that section is assumed to exist in the INF file specified by *InfHandle*.
